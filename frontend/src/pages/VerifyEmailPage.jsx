...
export function VerifyEmailPage() {
  ...
  async function handleSend() {
    ...
    setError('');
    try {
      const response = await verificationService.sendEmailVerification(accessToken);
      setSent(true);
  ...
  async function handleConfirm() {
    ...
    setError('');
    try {
      await verificationService.confirmEmailVerification(accessToken, code.trim());
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-blue-50 text-warelyn-primary">
              <Mail size={20} />
            </div>
            <div>
              <h2 className="text-lg font-bold tracking-tight text-warelyn-text">Verify Email</h2>
              <p className="text-xs text-warelyn-muted">Confirm your email address</p>
            </div>
          </div>
        </CardHeader>
        <CardBody>
          ...
          {!sent ? (
            ...
            <div className="space-y-4">
              <p className="text-sm text-warelyn-muted">Enter the 6-digit code sent to your email. Code expires in 10 minutes.</p>
