  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Badge tone={statusTone[order.status] ?? 'neutral'}>{order.status}</Badge>{mayWrite && order.status === 'DRAFT' ? <Button disabled={isSaving} onClick={confirm}>Confirm</Button> : null}{mayWrite && ['DRAFT', 'CONFIRMED', 'PARTIALLY_FULFILLED'].includes(order.status) ? <Button disabled={isSaving} variant="danger" onClick={() => runAction(salesService.cancelSalesOrder)}>Cancel</Button> : null}{mayWrite && fulfillableStatuses.has(order.status) ? <Button disabled={isSaving} variant="secondary" onClick={() => runAction(salesService.closeSalesOrder)}>Close</Button> : null}{fulfillableStatuses.has(order.status) ? <Link to={`/sales/${order.id}/pick`}><Button variant="secondary">Pick order</Button></Link> : null}{fulfillableStatuses.has(order.status) ? <Link to={`/sales/${order.id}/package`}><Button variant="secondary">Packages</Button></Link> : null}{mayWrite && fulfillableStatuses.has(order.status) ? <Link to={`/sales/${order.id}/fulfill`}><Button variant="accent">Fulfill</Button></Link> : null}{mayWrite && ['PARTIALLY_FULFILLED', 'FULFILLED', 'CLOSED'].includes(order.status) ? <Link to={`/returns/new?sales_order_id=${order.id}`}><Button variant="secondary">Create return</Button></Link> : null}</div>
      </div>
