...
export function BlockedStockReportPage() {
  return <SimpleReportPage title="Blocked stock" description="Non-sellable return, batch, and serial stock records from backend reports." load={reportsService.getBlockedStock} columns={columns} normalize={(row) => ({ ...row, created_at: new Date(row.created_at).toLocaleString() })} />;
}
