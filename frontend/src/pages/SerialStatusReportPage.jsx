...
export function SerialStatusReportPage() {
  return <SimpleReportPage title="Serial status" description="Backend serial-level status report." load={reportsService.getSerialStatus} columns={columns} normalize={(row) => ({ ...row, created_at: new Date(row.created_at).toLocaleString() })} />;
}
